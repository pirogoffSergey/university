...
- (void)drawCirlceChartWithRaduis:(CGFloat)r {
    ...
    CGPoint p1 = [self interpritateFuncToScreenX:0 y:0];
    
    [_canvas setImage:[self drawCircleAtPoint:p1 withRadius:r onImage:_canvas.image]];
}
...
- (UIImage *)drawCircleAtPoint:(CGPoint)pt withRadius:(CGFloat)r onImage:(UIImage *)img {
    ...
    CGContextAddEllipseInRect(currentContext, CGRectMake(pt.x-r/2, pt.y-r/2, r*5, r*5));
...
//        [self clearDotsLayer];
    ...
    [_dotsLayer setImage: [self lineFrom:pt to:CGPointMake(pt.x, pt.y) image:_dotsLayer.image withColor:[UIColor blueColor] lineWidth:10]];
}
